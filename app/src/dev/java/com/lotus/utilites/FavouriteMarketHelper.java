 ...
 * @Since 09/10/18
 */
public class FavouriteMarketHelper {
    public static final String TAG = FavouriteMarketHelper.class.getSimpleName();
    public static final int UPDATE_INTERVAL = 1000;
    public boolean isUpdaterStart = false;
    static FavouriteMarketHelper matchMarketHelper = null;
    private MainActivity context;
    private String market_ids;
    public static String inplayMarket_ids = "";
    private FavouriteMarketUpdaterListener favouriteMarketUpdaterListener;
    ...
    private FavouriteMarketHelper(MainActivity context) {
        this.context = context;
        webRequestHelper = new WebRequestHelper(context);
    }

    public static FavouriteMarketHelper getNewInstances(MainActivity context, String market_ids) {

        if (matchMarketHelper == null) {
            matchMarketHelper = new FavouriteMarketHelper(context);
        }
    ...
    public void setFavouriteMarketUpdaterListener(FavouriteMarketUpdaterListener favouriteMarketUpdaterListener) {
        this.favouriteMarketUpdaterListener = favouriteMarketUpdaterListener;
    }
    ...
    private void handleMarketIdsResponse(WebRequest webRequest) {
        ...
        if (context.isValidString(response)) {
            ...
            if (responseModel != null && context.getFavouriteFragment() != null) {
                ...
                Set<String> cricketRunningMarkets=new HashSet<>();
                Set<String> sccoreRunningMarkets=new HashSet<>();
                Set<String> tanisRunningMarkets=new HashSet<>();
...
//                    String eventTypeId = oddsByMarketIdsModel.getEventTypeId();
                    ...
                    for (MatchDetailModel matchDetailModel : newCricketList) {
                        ...
                        for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                            if (matchMarketModel.getMarketid().equals(oddsByMarketIdsModel.getId())) {
                                cricketRunningMarkets.add(oddsByMarketIdsModel.getId());
                    ...
                    if (!marketFound) {
                        ...
                        for (MatchDetailModel matchDetailModel : newTennisList) {
                            ...
                            for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                                if (matchMarketModel.getMarketid().equals(oddsByMarketIdsModel.getId())) {
                                    tanisRunningMarkets.add(oddsByMarketIdsModel.getId());
                    ...
                    if (!marketFound) {
                        ...
                        for (MatchDetailModel matchDetailModel : newSoccerList) {
                            ...
                            for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                                if (matchMarketModel.getMarketid().equals(oddsByMarketIdsModel.getId())) {
                                    sccoreRunningMarkets.add(oddsByMarketIdsModel.getId());
                ...
                for (MatchDetailModel matchDetailModel : newCricketList) {
                    for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                        if(!cricketRunningMarkets.contains(matchMarketModel.getMarketid())){
                ...
                for (MatchDetailModel matchDetailModel : newTennisList) {
                    for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                        if(!tanisRunningMarkets.contains(matchMarketModel.getMarketid())){
                            matchMarketModel.setStatus("SUSPENDED");
                        }
                    }
                }
                ...
                for (MatchDetailModel matchDetailModel : newSoccerList) {
                    for (MatchMarketModel matchMarketModel : matchDetailModel.getMarkets()) {
                        if(!sccoreRunningMarkets.contains(matchMarketModel.getMarketid())){
                            matchMarketModel.setStatus("SUSPENDED");
                        }
                    }
                }
                ...
                if (favouriteMarketUpdaterListener != null) {
                    if (context != null && !context.isFinishing()) {
                        context.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                if (favouriteMarketUpdaterListener != null) {
                                    favouriteMarketUpdaterListener.onMarketUpdate(newCricketList,
                                            newTennisList,
                                            newSoccerList);

                                    favouriteMarketUpdaterListener.onMatchListUpdateFromMarket(newCricketList, diffResultCricket,
    ...
    public void startFavouriteMarketUpdater() {
        stopFavouriteMarketUpdater(false);
        if (market_ids != null) {
            isUpdaterStart = true;
    ...
    public void stopFavouriteMarketUpdater(boolean needClean) {
    ...
    public interface FavouriteMarketUpdaterListener {
